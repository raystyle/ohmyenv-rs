...
- **锚定的目标**：D34 已交付（2026-09-10）：云端清单加 minisign 签名与内嵌公钥校验（本地与云端一起校验），
  私钥只在开发机与 CI 密钥库、其他机器零手工（公钥随二进制）；待推 main 首发云端签名件后做端到端验收。
...
- 当前目标：D34 已交付（2026-09-10）：待推 main 触发 seed-mirror 首发签名件，再做云端端到端验收。
...
| 日期 | 目标 | 结果 |
| --- | --- | --- |
| 2026-09-10 | D34 云端清单防 MITM 非对称校验 | 达成：minisign 签名加内嵌公钥（本地与云端一起校验），签名工具与 CI 签名流水就位；单测与门禁全绿，云端端到端验收待推送后执行 |
