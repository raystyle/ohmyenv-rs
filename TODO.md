...
| 签名播种流水 B 承接双轨切换（omc 回执选 B） | 待办 | 配合三件：`.tools/catalog-sign` 与公钥 omc 侧自行复制（本仓对其只读）；`CATALOG_SIGNING_KEY` Secret 值由用户灌入 raystyle/ohmycloud（私钥只在本机 `~/.config/ome/catalog-signing.key`）；对方流水首发并经本仓 `ome catalog sync` 验签 valid 后，本仓撤 Secret 与 seed-mirror 签名步（路线 A 自产段与 catalog_lint 发布门不移交） | 2026-09-10 |
