...
| manifest 对线共识五项（codex 三轮补审） | 待办 | ①③④⑤已落（①auto_refresh 三路径补 manifest 拉取加 mtime 时效，真机实证 fresh 补拉与离线不动；③post_install 失败降 stderr WARN 不拦收尾，幂等分支重跑成重试路径，R016 已补「命令须幂等」数据面约束；④win 超时改 taskkill /T 先行加 kill 加 wait，A/B 实证与用例断言齐；⑤orphan 反向 lint 由 omc 落第四面）。余项：②uv-git/npm-tgz 早退通道的应用点上提（迁 rmux/browser-harness/omc 时先行）、撤内建 parity 门、POSIX 无进程组杀（pre_exec setpgid 加 killpg，未落）。 | 2026-09-11 |
