...
| manifest 对线共识五项（codex 三轮补审） | 待办 | ①TTL 自动刷新不含 manifest（端上自动跟进时效）与③post_install 失败重试路径（完成标记或收尾统一报）：随 omc manifest 正式上线一起定；②uv-git/npm-tgz 早退通道的应用点上提：迁那两族（rmux/browser-harness/omc）时先行；④win kill 不杀进程树（taskkill /T 或 job object）排近期；⑤manifest orphan 节反向 lint 归 omc seed lint（随其 DSL lint 三面清单） | 2026-09-11 |
