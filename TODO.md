...
D37 全链终态并 v0.3.1 封版（2026-09-10）：codex 二轮对线修复（POSIX CI 红、路线 A 潜伏失败、七项收口加 M016 纪律），claude 复验 CI 三平台与本机全绿、对账 101/101、验签 valid 后封版；D38 消费面镜像直装已交付并封 v0.4.0（2026-09-11）；队列仅挂 v0.3.x 回滚防护。
...
| 回滚重放防护（S006 候选 B，定义先行） | 待办 | **防护场景定义**（ohmycloud 联署征询 2026-09-11）：一为「重放攻击」：攻击者或网络层把一份**旧的但签名有效**的 catalog/manifest 对（如含已下架含毒版本或旧 pin）推给端上，现三重门全过（签名没坏，只是旧）；二为「镜像回滚」：桶对象被回滚到旧版无版本化路径下，端上无法察觉比已见版本旧。解法：清单头加 generated_at 时间戳与单调递增 seq，端上 .last-sync 记已见 seq，收到更低 seq 拒收并告警；涉及 catalog schema 加字段（omc 数据面）与引擎拒收逻辑（ome），排期待用户裁决 | 2026-09-11 |
