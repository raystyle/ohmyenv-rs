...
| manifest 对线共识五项（codex 三轮补审） | 待办 | orphan 反向 lint 已由 omc 落第四面（2026-09-11 终报）；共识①③④已落码（auto_refresh 三路径补 manifest 拉取与 mtime 时效、post_install 失败降 WARN 加幂等分支重试、win 超时补 taskkill /T 杀树）真机实证 manifest fresh 补拉，余②应用点上提待迁族； ①TTL 自动刷新不含 manifest（端上自动跟进时效；可见性半已落 2026-09-11：catalog status manifest 面加 install/update 缺位 WARN）与③post_install 失败重试路径（完成标记或收尾统一报）：随 omc manifest 正式上线一起定；②uv-git/npm-tgz 早退通道的应用点上提：迁那两族（rmux/browser-harness/omc）时先行；④win kill 不杀进程树（taskkill /T 或 job object）排近期；⑤manifest orphan 节反向 lint 归 omc seed lint（随其 DSL lint 三面清单） | 2026-09-11 |
