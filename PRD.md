...
| D12 | 特型工具部署深诊（用户 2026-09-07「vsbuild 和 rust 这类应该可以深入诊断安装部署情况和部署后的状态」）：evergreen/安装器型工具的**部署产物核对**：rust（rustup toolchain stable 激活态、rustc/cargo 双在）、vsbuild（组件三件套实装：MSBuild.exe 与 VC\Tools\MSVC 在安装目录、机器级 PATH 注册态）；判据从 install/vsbuild.rs/rustup.rs 的写入动作与布局字段来（写入动作背书纪律的部署产物延伸） | 已采纳 | 第 0 轮（用户方向指令） | doctor check 节增量；与 config_health 同风格 |
