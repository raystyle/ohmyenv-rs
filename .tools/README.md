...
| 工具 | 用途 | 用法 |
| --- | --- | --- |
| `import-catalog.ps1` | 从 ohmypwsh `scripts\catalog.psd1`（Win 侧）+ `helpers.ps1` New-ToolDef 静态元数据生成 `catalog\tools.toml`（29 工具；psd1 只读不写；模式见 R001） | `pwsh -NoProfile -File .tools\import-catalog.ps1` |
