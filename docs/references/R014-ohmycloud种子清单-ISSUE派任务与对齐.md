...
6. **版本里程碑对齐**（D30，2026-09-10 omc 回执定稿）：2026-09-12 三仓同日封版（omc 0.3.0 / ome v0.2.0 首个 tag / oma v0.5.0）；重叠削减已落 ome 侧：doctor agent 层移除（装态对账归 omc、token 归 oma diagnose）、ome 自身装位与升级通道边界澄清（首次装位 omc tool 加 ome init 双通道同位，升级单通道 self update）；omc 调 ome install 契约冻结。另定协调项：yolo 托管端用户级以 omc patchCodexConfig 为准（oma 面向项目级）、pretrust 归 oma init（ome 无对应面，仅登记口径）。ome 首个 v* tag 触发镜像 ome/latest 拆 stable（ohmycloud 挂账执行，原 #8 状态盘点项）。
