...
> 角色：**选 Rust 库与项目的研究操作手册**。AGENTS 写 Rust 规则「先查最流行、最稳定的库」的细则载体。移植自 ohmyagents 同名细则（其两法均为 2026-08-31 本机实证：crates.io 通道与 GitHub 通道，完整证据链见 ohmyagents `docs\research\` S013）。Python 侧项目工具选库另见 `R008-项目工具Python库选型细则-pypi与uv.md`。
...
| 源码深读 | GitHub 通道 clone，后续接 rg / ast-grep / git |
...
稳度四信号：`max_stable_version` 非空、`recent_downloads` 高、`updated_at` 近 6 到 12 个月、反向依赖不低。阈值是启发式不是门禁：窄领域新库会被误杀，人工核 repository 与文档再定（0.x 但发布节奏勤的窄领域库，可配 pin `=x.y.z` 用）。
...
# 4 深读：先看历史再拿代码；clone 后用 rg / ast-grep / git 挖
...
`gh` 无 docs 子命令；正解是 code search 限定 markdown：

```bash
# 跨仓只扫 md（extension 必须内联；--filename "*.md" 通配不可靠）
gh search code "<概念词> extension:md" --limit 5

# GitHub 官方文档定点（docs.github.com 源仓；空结果 = 官方没写这概念）
...
| search 与 view 字段名不同 | `stargazersCount`（search）对 `stargazerCount`（view）；open 数取 `issues.totalCount` |
| 星数当唯一标准 | stars 与 pushedAt 并看；新秀另走 created 筛选 |
| `--filename` 通配 | 扩展名用内联 `extension:md`，filename 给具体名 |
