...
| `catalog\` | `catalog\tools.toml` 唯一 pin 源与静态字段权威（42 工具 = 37 加 agent 四家（D07）加 zoxide/sheldon（S004）加 ffmpeg（D14）加 rclone（D22，ohmycloud#9 请求）减 vault 与 browser-harness/reader（2026-09-07 用户裁）；D18 起无外部 catalog 对照） |
