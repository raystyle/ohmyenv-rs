...
| `catalog\` | `catalog\tools.toml` 唯一 pin 源与静态字段权威（44 工具 = 37 加 agent 四家（D07）加 zoxide/sheldon（S004）加 ffmpeg（D14）加 rclone（D22）加 browser-harness/reader（D23 重入册，撤 2026-09-07 暂不接管裁）减 vault；D18 起无外部 catalog 对照） |
