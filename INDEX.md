...
| `src\doctor.rs` | 核心诊断命令三层（D07）：系统层（os/arch/avx 指令集）、agent 层（四家二进制/版本/token 可用性，不取设置不读环境变量）、依赖层（九类分组统计）加环境错误与配置健康 check 节（D11 五项：bunfig/goproxy/cargo 镜像、rust 重定位、遥测开关，判据与 heal 同源；FAIL 即 exit 1 专属 check 节） |
